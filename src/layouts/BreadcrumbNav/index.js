...
function BreadcrumbNav() {
    ...
    useEffect(() => {
        setCrumbPaths((prev) => {
            const index = prev.findIndex((item) => item.pathname === pathname);
            // 如果该路由已存在，则同步更新它的 search（例如 /article/create?id=...）
            if (index >= 0) {
                const next = [...prev];
                next[index] = { pathname, search };
                return next;
            }
            // 否则将新路由追加到列表末尾
    ...
    const handleClose = useCallback((path, event) => {
        ...
        setCrumbPaths((prev) => {
            // 从列表中移除指定的路由路径
            const next = prev.filter((item) => item.pathname !== path);
            // 如果列表为空，则添加首页作为兜底
            if (next.length === 0) {
                next.push({ pathname: '/home', search: '' });
            }
                ...
                >
                    ...
                    {item.pathname !== '/home' ? (
                        <CloseOutlined
                            ...
                            onClick={(event) => handleClose(item.pathname, event)}
                        />
                    ) : null}
                </span>
            ),
